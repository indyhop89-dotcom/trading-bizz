/**
 * Kamakhya Loyalties' Excel export — same Excel-2003 SpreadsheetML technique
 * as documentExcel.js's "vananam" family builder, headered in the same navy
 * used throughout kamakhyaDocumentTemplate.js's print layout.
 */
...
export function buildKamakhyaDocumentExcelXML(doc, theme) {
  const {
  ...
  // — same convention as documentTemplate.js's buildDocumentHTML.
  ...
  const headers = ['#', 'Item & Description', 'HSN/SAC', 'Qty', 'Unit', 'Rate', 'GST %', interstate ? 'IGST' : 'GST Amt (CGST+SGST)', 'Taxable Amount']
  ...
  lines.forEach((l, i) => {
    ...
    rows += `<Row ss:Height="16">${cell(i + 1)}${cell(l.description || '')}${cell(l.hsn_code || '')}` +
      ...
      `<Cell><Data ss:Type="Number">${Number(l.taxable_amount) || 0}</Data></Cell></Row>`
  })

  rows += `<Row ss:Height="6"/>`
  rows += `<Row>${emptyCells(6)}${cell('Sub Total', true)}${cell(Number(totals.taxable_amount) || 0, false, 'num')}${emptyCells(1)}</Row>`
  if (interstate) {
    rows += `<Row>${emptyCells(6)}${cell('IGST', true)}${cell(Number(totals.igst_amount) || 0, false, 'num')}${emptyCells(1)}</Row>`
  } else {
    rows += `<Row>${emptyCells(6)}${cell('CGST', true)}${cell(Number(totals.cgst_amount) || 0, false, 'num')}${emptyCells(1)}</Row>`
    rows += `<Row>${emptyCells(6)}${cell('SGST', true)}${cell(Number(totals.sgst_amount) || 0, false, 'num')}${emptyCells(1)}</Row>`
  }
  rows += `<Row>${emptyCells(6)}${cell('Round Off', true)}${cell(Number(totals.round_off_amount) || 0, false, 'num')}${emptyCells(1)}</Row>`
  rows += `<Row>${emptyCells(6)}${cell('Total', true)}${cell(Number(totals.total_amount) || 0, true, 'total')}${emptyCells(1)}</Row>`
  rows += `<Row ss:Height="6"/>`
  if (docType !== 'PO' && (bankDetails.bank_name || bankDetails.bank_account_no)) {
    rows += `<Row>${cell(`Bank: ${bankDetails.bank_name || '—'}  |  A/C: ${bankDetails.bank_account_no || '—'}  |  IFSC: ${bankDetails.bank_ifsc || '—'}`)}${emptyCells(8)}</Row>`
  }
  ...
  return `<?xml version="1.0" encoding="UTF-8"?><?mso-application progid="Excel.Sheet"?><Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet"><Styles>` +
    ...
    `<Style ss:ID="6"><Font ss:Bold="1" ss:Color="#FFFFFF"/><Interior ss:Color="${navy}" ss:Pattern="Solid"/><NumberFormat ss:Format="#,##0.00"/></Style>` +
    `<Style ss:ID="7"><Font ss:Bold="1" ss:Size="13"/></Style>` +
    `<Style ss:ID="hdr"><Font ss:Bold="1" ss:Color="#FFFFFF"/><Interior ss:Color="${navy}" ss:Pattern="Solid"/></Style>` +
    `</Styles><Worksheet ss:Name="${xesc(meta.title)}"><Table ss:DefaultColumnWidth="80" ss:DefaultRowHeight="16">` +
    `<Column ss:Width="28"/><Column ss:Width="200"/><Column ss:Width="68"/><Column ss:Width="48"/><Column ss:Width="48"/><Column ss:Width="70"/><Column ss:Width="48"/><Column ss:Width="88"/><Column ss:Width="88"/>` +
